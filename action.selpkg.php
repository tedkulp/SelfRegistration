...
// we're allowing the user to select his own package (group).
if( isset($params['selfreg_submit']) && isset($params['selfreg_pkg']) )
  {
    $pkgid = (int)$params['selfreg_pkg'];
    $query = 'SELECT * FROM '.cms_db_prefix().'module_selfreg_paidpkgs WHERE id = ?';
    $row = $db->GetRow($query,array($pkgid));
    if( !$row )
      {
    ...
    $ginfo = $feu->GetGroupInfo($row['gid']);
    if( $ginfo[0] === FALSE )
      {
	// couldn't find the package.
	die('problem 2');
    ...
    $group = $ginfo['groupname'];

    $parms = array('group'=>$group);
