...
if( is_array($packagelist) )
  {
    for( $i = 0; $i < count($packagelist); $i++ )
      {
	$rec =& $packagelist[$i];
	$rec['edit_url'] = $this->CreateURL($id,'admin_addpaidpkg',$returnid,
					    array('pkgid'=>$rec['id']));
	$rec['delete_link'] = $this->CreateImageLink($id,'admin_delpaidpkg',$returnid,
						     $this->Lang('delete'),
						     'icons/system/delete.gif',
						     array('pkgid'=>$rec['id']));
	$rec['edit_link'] = $this->CreateImageLink($id,'admin_addpaidpkg',$returnid,
						     $this->Lang('edit'),
...
$smarty->assign('addlink',$this->CreateImageLink($id,'admin_addpaidpkg',$returnid,
						 $this->Lang('add_paidpkg'),
