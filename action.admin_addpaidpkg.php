...
if( isset($params['submit']) )
  {
    ...
    // check for errors.
    $flds = array('name'=>$this->Lang('name'),
		  'prompt'=>$this->Lang('prompt'));
    //		  'gid'=>$this->Lang('group'));
    foreach( $flds as $fld => $lbl )
      {
	$query = sprintf('SELECT id FROM '.cms_db_prefix().'module_selfreg_paidpkgs
                          WHERE %s = ?',$fld);
	$parms = array($rec[$fld]);
	if( $pkgid )
	  {
...
$smarty->assign('input_description',$this->CreateTextArea(true,$id,$rec['description'],
							  'pkg_description'));
$subscr_type = array();
